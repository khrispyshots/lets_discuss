...
const App = () => {
  ...
  useEffect(() => {
    ...
    if( networks[chainId?.toString(16)]?.includes('WDOGE') ){
      fetchMints();
    }
    ...
    if (account) {
      try {
        ...
        await library.send(
          ...
          [{ chainId: '568' }] // Check networks.js for hexadecimal network ids
        );
      } catch (error: any) {
  ...
  const renderInputForm = () => {
    ...
    //if (networks[chainId?.toString(16)] !== 'Dogechain Testnet') {
      return (
          ...
          <>
            <span id="addr" className="record">
              <input
                ...
                placeholder="whats ur Doggy power"
                readOnly={true}
                className="readonly"
              />
            </span>
